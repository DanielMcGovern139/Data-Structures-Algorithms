//linear probing using a better hash function
...
Element* hashTable[ARRAY_SIZE]; //hash table of size 57..can store 67 names 
...
int hash2(char* s){
    ...
    hash = hash*2; //MULTIPLIED BY A PRIME 
    //hash = hash*3;
    s++;
    }
...
Element* createNewElement(char* name){
    ...
    strcpy(new_element->surname, name); //copying name into the new element
    new_element->count = 1;
    return new_element;
}
...
Element* search (char* name){
    ...
    int i = 0;
    int hash = (hash2(name)+i)%ARRAY_SIZE;
    
    while(hashTable[hash]!=NULL){ //once its null we stop the loop and return null
      
     if (strcmp(hashTable[hash]->surname,name)==0){ //if two strings are the same it returns 0..otherwise it wil return a non zero value (if surname = name)
      return hashTable[hash];
      
     }
     i++;
      hash = (hash2(name)+i)%ARRAY_SIZE; //move onto the next hash key
    }
    return NULL;
}
