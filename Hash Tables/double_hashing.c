//hash table to store the frequency of names in an unordered list of names using char arrays as keys.  This method is implemented using two hash functions instead of one
...
Element* search (char* name){
    ...
    while(hashTable[hash]!=NULL){ //once its null we stop the loop and return null
     ...
     i++;
      hash = (hash_function(name)+i*hash3_function(name))%ARRAY_SIZE; //move onto the next hash key
    }
    return NULL;
}
